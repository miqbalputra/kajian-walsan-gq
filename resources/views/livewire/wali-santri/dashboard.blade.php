<div class="min-h-screen flex flex-col pb-20">
    ...
    {{-- Banner: Ajakan hubungkan Google (hanya jika belum link & Google dikonfigurasi) --}}
    @if(config('services.google.client_id') && !auth()->user()->hasGoogleLinked())
        <div class="px-4 mt-2" x-data="{ show: !localStorage.getItem('dismiss-google-banner-{{ auth()->id() }}') }" x-show="show" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95">
            <div class="bg-gradient-to-r from-blue-500 to-indigo-600 rounded-3xl shadow-xl p-5 text-white flex gap-4 items-center relative overflow-hidden group">
                <div class="absolute -right-4 -bottom-4 w-28 h-28 bg-white/10 rounded-full blur-2xl group-hover:scale-125 transition-transform duration-700 z-0"></div>
                <div class="w-12 h-12 bg-white/20 backdrop-blur-md rounded-2xl flex items-center justify-center shrink-0 border border-white/30 relative z-10">
                    <svg class="w-6 h-6" viewBox="0 0 24 24"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#fff"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#fff" opacity=".7"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#fff" opacity=".5"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#fff" opacity=".8"/></svg>
                </div>
                <div class="flex-1 relative z-10">
                    <p class="text-white/80 text-[10px] font-black uppercase tracking-[0.2em] mb-0.5">Login Lebih Mudah</p>
                    <h3 class="font-bold text-sm leading-tight">Hubungkan akun Google Anda agar tidak perlu ingat password</h3>
                </div>
                <div class="flex flex-col gap-2 shrink-0 relative z-10">
                    <button @click="show = false; localStorage.setItem('dismiss-google-banner-{{ auth()->id() }}', 'true')"
                        class="p-1 hover:bg-white/10 rounded-full transition-colors text-white/50 hover:text-white" title="Tutup">
                        <span class="material-symbols-rounded text-sm">close</span>
                    </button>
                    <a href="{{ route('google.link') }}" class="px-4 py-2 bg-white text-indigo-600 rounded-xl font-bold text-xs shadow-lg hover:bg-indigo-50 transition-all active:scale-95">
                        Hubungkan
                    </a>
                </div>
            </div>
