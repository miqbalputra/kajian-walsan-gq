<div>
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Presensi Wali Santri</h1>
            <p class="text-gray-500 dark:text-gray-400">Pantau performa wali santri tertentu berdasarkan nama wali, santri, kelas, atau kajian.</p>
        </div>
    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div>
                ...
                <select wire:model.live="kajianId"
                    class="w-full px-4 py-2.5 border border-gray-200 dark:border-slate-700 dark:bg-slate-950 dark:text-white rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                    <option value="">Semua Kajian Tercatat</option>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Cari Wali/Santri</label>
                <input type="text" wire:model.live.debounce.300ms="search"
                    class="w-full px-4 py-2.5 border border-gray-200 dark:border-slate-700 dark:bg-slate-950 dark:text-white rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                    placeholder="Nama wali, santri, kelas...">
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800 overflow-hidden mb-6">
        <div class="p-4 border-b border-gray-100 dark:border-slate-800">
            <h2 class="font-bold text-gray-900 dark:text-white">Performa Wali Santri Berdasarkan Rentang Tanggal</h2>
            <p class="text-sm text-gray-500 mt-1">Gunakan kolom cari untuk fokus ke wali santri atau santri tertentu.</p>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full min-w-[1040px]">
                <thead class="bg-gray-50 dark:bg-slate-800/50 border-b border-gray-100 dark:border-slate-800">
                    <tr>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider">Wali Santri</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-slate-800">
                    @forelse($performanceRows as $row)
                        <tr class="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                            <td class="px-4 py-4">
                                <p class="font-semibold text-gray-900 dark:text-white">{{ $row['guardian_name'] }}</p>
                                <p class="text-xs text-gray-500">{{ $row['guardian_type'] }}</p>
                                <p class="text-xs text-gray-400">{{ $row['children'] }}</p>
                            </td>
                            <td class="px-4 py-4">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800 overflow-hidden">
        <div class="p-4 border-b border-gray-100 dark:border-slate-800">
            <h2 class="font-bold text-gray-900 dark:text-white">Detail Presensi Wali Santri</h2>
            <p class="text-sm text-gray-500 mt-1">
                @if($this->selectedKajian)
                    Menampilkan semua wali santri pada kajian: {{ $this->selectedKajian->title }}.
                @else
                    Menampilkan data presensi wali santri yang tercatat dalam rentang tanggal.
                @endif
            </p>
        </div>
        <div class="overflow-x-auto">
                    ...
                    @forelse($rows as $row)
                        <tr class="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                            ...
                            <td class="px-4 py-4">
                                ...
                                @else
                                    <span class="text-sm text-gray-400 font-medium">Tidak ada file</span>
                                @endif
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-12 text-center text-gray-500">
                                ...
                                <p class="mt-2">Tidak ada data presensi wali santri.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
